...
/**
 * Send message to WeChat Work using webhook
 * @param {Object} config - WeChat configuration
 * @param {string} config.webhookUrl - WeChat Work webhook URL
 * @param {string} message - Message text
 */
async function sendMessage(config, message) {
    const { webhookUrl, timeout = 10000 } = config
    ...
    try {
        ...
        const errorMessage =
            sendError.response && sendError.response.data
                ? sendError.response.data.errmsg ||
                  sendError.response.data.message
                : sendError.message
        throw new Error(`WeChat send failed: ${errorMessage}`)
    }
}

/**
 * Format message for WeChat Work with markdown support
 */
